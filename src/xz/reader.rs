...
impl XzStream {
            ...
            match &self.state {
                ...
                XzStreamState::Lzma2Data => {
                    if self.filter.is_some() {
                        let n = self.process_lzma2_filtered(
                        ...
                        if lzma2.is_draining() {
                            ...
                            lzma2.drain_with_filter(output, &mut out_pos);
                ...
                _ => {
                    if self.accum.len() < self.accum_needed {
                        if in_pos >= input.len() {
                            if action == Action::Finish && input.is_empty() {
    ...
    fn process_lzma2_filtered(
        ...
        // Step 1: Copy already-filtered bytes from filter_buf to output
        if self.filter_pos < self.filter_buf.len() - self.filter_unfiltered {
            ...
            *out_pos += n;
            ...
            if self.filter_pos < ready_end {
                // Output full — but we must still drain LZMA2 and consume
                // input so the caller doesn't lose bytes.
                let lzma2 = self.lzma2.as_mut().unwrap();
                while lzma2.is_draining() {
                    ...
                    let filtered =
                        self.filter.as_mut().unwrap().apply_decode(filter_slice);
                    self.filter_unfiltered = filter_slice.len() - filtered;
                }
                if *in_pos < input.len() && !lzma2.is_draining() && !lzma2.is_finished()
                {
                    let result = lzma2.process(&input[*in_pos..], &mut [], action)?;
                    *in_pos += result.bytes_consumed;
                    self.total_in += result.bytes_consumed as u64;
                }
                return Ok(0); // output full
            }
        ...
        // Step 2: Drain LZMA2 decoded data into filter_buf, then apply filter
        if lzma2.is_draining() {
            let prev_len = self.filter_buf.len();
            lzma2.drain_to_buf(&mut self.filter_buf, 4096);
            let new_bytes = self.filter_buf.len() - prev_len;
        ...
        // Step 3: Feed input to LZMA2 with no output buffer (it'll enter drain)
        let result = lzma2.process(&input[*in_pos..], &mut [], action)?;
        *in_pos += result.bytes_consumed;
        self.total_in += result.bytes_consumed as u64;
        ...
        if *in_pos >= input.len() && !lzma2.is_draining() {
            return Ok(0);
        }
        Ok(1)
    }
    ...
    fn process_accumulated(&mut self) -> Result<()> {
        match &self.state {
            ...
            XzStreamState::BlockChecksum { remaining } => {
                ...
                let checksum = self.checksum.take().unwrap();
            ...
            XzStreamState::IndexRecordUnpadded { remaining } => {
                ...
                let _unpadded = parse_multibyte_integer(&self.accum)?;
                let vli_size = count_multibyte_integer_size(&self.accum);
                self.index_crc.update(&self.accum[..vli_size]);
                self.index_size += vli_size;
            ...
            XzStreamState::IndexRecordUncompressed { remaining } => {
                ...
                let _uncompressed = parse_multibyte_integer(&self.accum)?;
                let vli_size = count_multibyte_integer_size(&self.accum);
                self.index_crc.update(&self.accum[..vli_size]);
                self.index_size += vli_size;
            ...
            XzStreamState::InterStreamPadding => {
                let data = &self.accum;
                if data.len() < 4 {
                    self.accum_needed = 4;
                    return Ok(());
                ...
                if data[..4] == [0, 0, 0, 0] {
                    self.accum.clear();
                    self.accum_needed = 4;
                } else if data[..6.min(data.len())] == XZ_MAGIC[..data.len().min(6)] {
