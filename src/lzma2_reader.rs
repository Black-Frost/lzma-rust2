...
impl Lzma2Stream {
    ...
    pub fn new(dict_size: u32) -> Self {
        let dict_size = get_dict_size(dict_size) as usize;
        Self {
    ...
    fn process_chunk_header(&mut self) -> crate::Result<()> {
        ...
        if first_byte >= 0x80 {
            let needs_props = first_byte >= 0xC0;
            let needed = if needs_props { 6 } else { 5 };
            if self.accum.len() < needed {
                self.accum_needed = needed;
                return Ok(());
            }

            let control = first_byte;
